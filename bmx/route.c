...
STATIC_FUNC
void configure_rule( uint32_t network, int16_t mask, int16_t rt_table_macro, uint32_t prio, char* iif,
                          int16_t rta_type, int8_t del, int8_t track_t)
{
	...
	inet_ntop( AF_INET, &network, str1, sizeof (str1) );
        ...
        if (track_rule_and_proceed(network, mask, prio, rt_table, iif, rta_type, del, track_t) == NO)
                return;
	             ...
	             (struct sockaddr *)&nladdr, sizeof(struct sockaddr_nl) ) < 0 ) 
	{
        ...
        do {
                ...
                dbgf_all(DBGT_INFO, "searching: %s  NLMSG_OK: %d  len: %zd",
                        is_rule ? "rules" : "routes", NLMSG_OK(nh, len), len);
                ...
                while (NLMSG_OK(nh, len)) {
                        ...
                        if ( /* rtm->rtm_table == 0 || */
                                ...
                                if (is_rule) {
                                        ...
                                        configure_rule( dest, mask, rtm->rtm_table, prio, iif, rule_type, DEL, TRACK_NO);
                                        ...
                                        configure_route(dest, mask, prio, router, 0, oif, "unknown", rtm->rtm_table, rtm->rtm_type, DEL, TRACK_NO);
...
static void restore_kernel_config ( struct dev_node *dev_node ) {
	...
	if ( dev_node ) {
		...
		if (dev_node->send_redirects_orig > -1) {
			sprintf( filename, "net/ipv4/conf/%s/send_redirects", dev_node->name_phy);
...
static int8_t dev_validate( uint8_t set, struct dev_node *bif, char* dev_name, uint8_t reduced ) {
	...
	} else if ( bif->ip4_addr != ((struct sockaddr_in *)&int_req.ifr_addr)->sin_addr.s_addr ) {
	...
	if ( set ) {
		...
		bif->ip4_netaddr = ( ((struct sockaddr_in *)&int_req.ifr_addr)->sin_addr.s_addr & bif->ip4_addr );
		
	} else if ( bif->ip4_netaddr != ( ((struct sockaddr_in *)&int_req.ifr_addr)->sin_addr.s_addr & bif->ip4_addr ) ) {
	...
	if ( set ) {
		...
		bif->ip4_netmask = ((struct sockaddr_in *)&int_req.ifr_addr)->sin_addr.s_addr;
		bif->ip4_prefix_length = bits_count( bif->ip4_netmask );
		
	} else if ( bif->ip4_netmask != ((struct sockaddr_in *)&int_req.ifr_addr)->sin_addr.s_addr ) {
	...
	if ( set ) {
		...
		bif->ip4_broad = ((struct sockaddr_in *)&int_req.ifr_broadaddr)->sin_addr.s_addr;
		
	} else if ( bif->ip4_broad != ((struct sockaddr_in *)&int_req.ifr_broadaddr)->sin_addr.s_addr ) {
...
static void dev_reconfigure_soft( struct dev_node *bif) {
	...
	if ( !bif->active ) {
		...
		dbg( DBGL_SYS, DBGT_INFO, 
		     ...
		     ARG_DEV, bif->name, ipStr(bif->ip4_addr), bif->ip4_prefix_length, ipStr(bif->ip4_broad) );
		
	} else if ( bif->linklayer == VAL_DEV_LL_WLAN  ) {
		
		dbg( DBGL_SYS, DBGT_INFO, 
		     "enabled wireless %s %s %s/%d brc %s (use %s /l=%d to optimize for ethernet)", 
		     ARG_DEV, bif->name, ipStr(bif->ip4_addr), bif->ip4_prefix_length, ipStr(bif->ip4_broad), bif->name, VAL_DEV_LL_LAN );
		...
		dbg( DBGL_SYS, DBGT_INFO, 
		     ...
		     ARG_DEV, bif->name, ipStr(bif->ip4_addr), bif->ip4_prefix_length, ipStr(bif->ip4_broad), bif->name, VAL_DEV_LL_WLAN );
...
static void dev_activate( struct dev_node *bif ) {
	...
	if ( bif->linklayer != VAL_DEV_LL_LO  &&  (bif->unicast_sock = socket( PF_INET, SOCK_DGRAM, 0 )) < 0 ) {
	...
	if ( bif->linklayer == VAL_DEV_LL_LO ) {
		...
		if ( setsockopt( bif->unicast_sock, SOL_SOCKET, SO_BROADCAST, &set_on, sizeof(set_on) ) < 0 ) {
		...
		if ( bind( bif->unicast_sock, (struct sockaddr *)&bif->ip4_unicast_addr, sizeof(struct sockaddr_in) ) < 0 ) {
...
#ifdef SO_TIMESTAMP
		if (setsockopt(bif->unicast_sock, SOL_SOCKET, SO_TIMESTAMP, &set_on, sizeof(set_on)))
		...
		if ( bind( bif->netwbrc_sock, (struct sockaddr *)&if_netwbrc_addr, sizeof(struct sockaddr_in) ) < 0 ) {
		...
		// bind recv socket to address
		if ( bind( bif->fullbrc_sock, (struct sockaddr *)&if_fullbrc_addr, sizeof(struct sockaddr_in) ) < 0 ) {
	...
	bif->active = YES;
        ...
        if (avl_find(&dev_ip4_tree, &bif->ip4_addr)) {
                dbgf(DBGL_SYS, DBGT_ERR, "%s already in dev_tree_ip4!", bif->ip4_str);
        } else {
                              ...
                              int16_t rt_table, int16_t rta_type, int8_t del, int8_t track_t )
{
        ...
        struct list_node *list_pos, *tmp_pos, *prev_pos = (struct list_node*)&routes_list;
                    ...
                    int16_t rt_table_macro, int16_t rta_type, int8_t del, int8_t track_t )
{
	...
	if ( del ) {
		...
		req.rtm.rtm_protocol = RTPROT_STATIC; // may be changed to some bmx specific value - see <linux/rtnetlink.h>
...
//                memset( &bif, 0, sizeof(struct dev_node));
		...
		if ( dev_validate( YES/*set*/, &bif, ifr->ifr_name, YES/*reduced check only*/ ) == FAILURE )
			continue;
                ...
                dbgf_ext(DBGT_INFO, "%32ju %32ju  %10s %15s %15s %2d ",
                        ...
                        ifr->ifr_name,  ipStr(bif.ip4_addr), ipStr(bif.ip4_netaddr), bif.ip4_prefix_length);
		...
		if( prio_rules && setup_tunnel == YES ) {
			...
			if ( Lo_rule && strncmp( ifr->ifr_name, "lo", strlen("lo") ) == 0 ) {

				configure_rule( 0, 0, RT_TABLE_TUNNEL, RT_PRIO_TUNNEL, "lo", RTA_IIF, ADD, TRACK_MY_NET );
                        }
...
void dev_deactivate( struct dev_node *bif ) {
	...
	dbg_mute( 30, DBGL_SYS, DBGT_WARN, "deactivating IF %-10s %-15s", bif->name, ipStr(bif->ip4_addr) );
	...
	if ( bif->linklayer != VAL_DEV_LL_LO ) {
        ...
        if (!bif->ip4_tree_addr) {
                dbgf(DBGL_SYS, DBGT_ERR, "no address given to remove in dev_tree_ip4!");
        } else if (!avl_find(&dev_ip4_tree, &bif->ip4_tree_addr)) {
                dbgf(DBGL_SYS, DBGT_ERR, "%s not in dev_tree_ip4!", ipStr(bif->ip4_tree_addr));
        } else {
...
void dev_check() {
	...
	Mtu_min = MAX_MTU;
		...
		if ((bif->active) && (!dev_up(bif->name))) {
			...
			if ( if_conf_hard_changed  ||  bif->hard_conf_changed  ||
                ...
                if (if_conf_soft_changed )
                        bif->soft_conf_changed = YES; // to be considered in if_activate()->if_reconfigure_soft()
		
		if ( (!bif->active)  &&  (dev_up(bif->name)) ) {

                        struct dev_node *tmp_bif = avl_find_item(&dev_ip4_tree, &bif->ip4_addr);

                        if (tmp_bif && !wordsEqual(tmp_bif->name, bif->name)) {

                                dbg_mute(40, DBGL_SYS, DBGT_ERR, "IF %-10s IP %-15s already used for IF %s",
                                        bif->name, bif->ip4_str, tmp_bif->name);
                                break;
		...
		if ( /*bif->if_active  &&*/ ( /*if_conf_soft_changed  ||*/  bif->soft_conf_changed) ) {
			
			if ( on_the_fly )
				dbg( DBGL_CHANGES, DBGT_INFO, "%s soft interface configuration changed", bif->name );
		...
		if ( !on_the_fly  &&  !bif->active ) {
			...
			dbg( DBGL_SYS, DBGT_WARN,
			     "not using interface %s (retrying later): interface not ready", bif->name);
		}
	}
...
static int32_t opt_policy_rt ( uint8_t cmd, uint8_t _save, struct opt_type *opt, struct opt_parent *patch, struct ctrl_node *cn ) {
	...
	if ( cmd == OPT_APPLY ) {
		...
		// add rule for hosts and announced interfaces and networks
		if ( prio_rules ) { 
			configure_rule( 0, 0, RT_TABLE_HOSTS,      RT_PRIO_HOSTS,      0, RTA_DST, ADD, TRACK_STANDARD );
			configure_rule( 0, 0, RT_TABLE_NETWORKS,   RT_PRIO_NETWORKS,   0, RTA_DST, ADD, TRACK_STANDARD );
		}
...
//        ord parent long_name          shrt Attributes				*ival		min		max		default		*func,*syntax,*help
	...
	{ODI,0,ARG_BASE_PORT,		0,  4,A_PS1,A_ADM,A_INI,A_CFA,A_ANY,	&base_port,	MIN_BASE_PORT,	MAX_BASE_PORT,	DEF_BASE_PORT,	0,
			ARG_VALUE_FORM,	"set udp ports"},
		
	{ODI,0,ARG_RT_PRIO,		0,  4,A_PS1,A_ADM,A_INI,A_CFA,A_ANY,	&Rt_prio,	MIN_RT_PRIO,	MAX_RT_PRIO,	DEF_RT_PRIO,	0,
			ARG_VALUE_FORM,	"set preferences for iproute2-style rules to rt_table (see: man ip)"},
		
	{ODI,0,ARG_RT_TABLE,		0,  4,A_PS1,A_ADM,A_INI,A_CFA,A_ANY,	&Rt_table,	MIN_RT_TABLE,	MAX_RT_TABLE-RT_TABLE_MAX_OFFS,	DEF_RT_TABLE,	0,
